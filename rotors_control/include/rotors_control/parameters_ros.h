/*
 * Copyright 2020 Ria Sonecha, Massachusetts Institute of Technology in
 Cambridge, MA, USA
...
template <typename T>
inline void GetRosParameter(const ros::NodeHandle& nh, const std::string& key,
...
inline void GetRotorConfiguration(const ros::NodeHandle& nh,
    ...
    unsigned int i = 0;
    while (nh.getParam(rotor_configuration_string + std::to_string(i),
                       single_rotor)) {
        if (i == 0) {
        ...
        nh.getParam(rotor_configuration_string + std::to_string(i) + "/angle",
                    rotor.angle);
        nh.getParam(
            rotor_configuration_string + std::to_string(i) + "/arm_length",
            rotor.arm_length);
        nh.getParam(rotor_configuration_string + std::to_string(i) +
        ...
        nh.getParam(
            rotor_configuration_string + std::to_string(i) + "/direction",
            rotor.direction);
...
inline void GetVehicleParameters(const ros::NodeHandle& nh,
...
inline void GetFullVehicleParameters(const ros::NodeHandle& nh,
                                     VehicleParameters* vehicle_parameters) {
    GetRosParameter(nh, "mass", vehicle_parameters->mass_,
                    &vehicle_parameters->mass_);
    GetRosParameter(nh, "inertia/xx", vehicle_parameters->inertia_(0, 0),
    ...
    GetRosParameter(nh, "inertia/zz", vehicle_parameters->inertia_(2, 2),
                    &vehicle_parameters->inertia_(2, 2));
    GetRosParameter(nh, "bf", vehicle_parameters->bf_,
                    &vehicle_parameters->bf_);
    GetRosParameter(nh, "bm", vehicle_parameters->bm_,
                    &vehicle_parameters->bm_);
    GetRosParameter(nh, "l", vehicle_parameters->armLength_,
